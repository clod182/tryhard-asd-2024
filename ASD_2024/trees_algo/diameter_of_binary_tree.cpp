...
int findDiameter(Node* root) {
    ...
    int leftHeight = findMaxDepthRecursive(root->left);
    int rightHeight = findMaxDepthRecursive(root->right);
    return max(leftHeight + rightHeight , max(findDiameter(root->left), findDiameter(root->right)));
}

// questa funzione ottimizzata ha TC di O(n) contro O(n^2) di quella non ottimizzata
int findMaxPathSum(Node* root, int& cur_max) {
    if(root == NULL) return 0;

    int leftDiameter = findMaxPathSum(root->left, cur_max);
    int rightDiameter = findMaxPathSum(root->right, cur_max);
    cur_max = max(cur_max, leftDiameter + rightDiameter);
    return 1 + max(leftDiameter, rightDiameter);
}

int findDiameterOptimized(Node* root) {
    int cur_max = 0;
    findMaxPathSum(root, cur_max);
    return cur_max;
}
...
int main() {
    ...
    int diam = findDiameter(root1);
    cout << "Diameter of Tree 1: " << diam << endl;
    ...
    diam = findDiameter(root2);
    cout << "Diameter of Tree 2: " << diam << endl;

    diam = findDiameterOptimized(root1);
    cout << "Diameter OPTIMIZED of Tree 1: " << diam << endl;
    diam = findDiameterOptimized(root2);
    cout << "Diameter OPTIMIZED of Tree 2: " << diam << endl;
