...
bool checkTreesIdentical(Node* root1, Node* root2) {
    if (root1 == NULL && root2 == NULL) {
        return true;
    }
    if (root1->data== root2->data && checkTreesIdentical(root1->left, root2->left) && checkTreesIdentical(root1->right, root2->right)) {
        return true;
    }
    return false;
}
...
int main() {
    ...
    bool check_identical = checkTreesIdentical(root1, root2);
    if(check_identical) cout << "Tree 1 and Tree 2 are identical? : YES" << endl;
    else cout << "Tree 1 and Tree 2 are identical? : NO" << endl;
