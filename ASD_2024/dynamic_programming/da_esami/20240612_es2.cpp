...
// ------------------ E' COME IL PROBLEMA "Longest_Increasing_Subsequence" ------------------

//---------------------------------------------------------------- MEMONIZATION top-down
int percorso_piu_lungo_helper1(vector<int>& alture, int index, vector<int>& dp) {
...
int percorso_piu_lungo1(vector<int>& alture) {
    int n = alture.size();
    vector<int> dp(n, -1);
    int max_length = 0;
    
    for (int i = 0; i < n; i++) {
        max_length = max(max_length, percorso_piu_lungo_helper1(alture, i, dp));
    }
...
int percorso_piu_lungo(vector<int>& alture) {
    ...
    vector<int> dp(n, 1);  // Inizializziamo dp con 1, poiché il minimo percorso ha lunghezza 1
    int max_length_top = 1;
    ...
    for (int i = 1; i < n; i++) {
        ...
        max_length_top = max(max_length_top, dp[i]);
    }
    
    return max_length_top;
}
