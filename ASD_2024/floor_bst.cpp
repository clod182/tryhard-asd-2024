...
void findFloorHelper(TreeNode* root, int input, int& floor) {
    if (root == NULL) return;
    if(root->val == input){
        floor = root->val;
        return;
    ...
    if (root->val < input) {
        ...
        findFloorHelper(root->right, input, floor);
    } else {
        findFloorHelper(root->left, input, floor);
    }  
}
int findFloor(TreeNode* root, int input) {
    if (root == NULL) return -1;
    int ret = INT_MIN;
    findFloorHelper(root, input, ret);
    if(ret == INT_MIN) return -1;
    return ret;    
}

int findFloorIterative(TreeNode* root, int input) {
    if (root == NULL) return -1;
    int ret = -1;
...
int main() {
    ...
    int res = findFloor(T2, 10);
    cout << "findFloor: " << res << "\n";

    res = findFloorIterative(T2, 10);
    cout << "findFloor ITERATIVE: " << res << "\n";
    return 0;
}